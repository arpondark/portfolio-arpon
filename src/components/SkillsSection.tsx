...
export default function SkillsSection() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ y }}
        className="absolute top-1/3 -left-48 w-96 h-96 rounded-full bg-gradient-to-br from-purple-500/20 to-transparent blur-3xl"
      />
      <motion.div
        style={{ y: useTransform(scrollYProgress, [0, 1], [-50, 50]) }}
        className="absolute bottom-1/3 -right-48 w-96 h-96 rounded-full bg-gradient-to-br from-pink-500/15 to-transparent blur-3xl"
      />
      ...
      <div className="container-custom relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-sm text-purple-300">My Expertise</span>
          </motion.div>
        ...
        {/* Mobile: Swipeable Category Indicator */}
        <div className="md:hidden mb-6">
          <div className="flex items-center justify-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-500/30">
              ...
              <span className="text-white font-medium">{skills[activeCategory].title}</span>
            </div>
        ...
        <div className="hidden md:block">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className={`p-3 rounded-xl bg-gradient-to-r ${skills[activeCategory].gradient}`}>
                {skills[activeCategory].icon}
              </div>
              <div>
                <h3 className="text-2xl font-bold text-white">{skills[activeCategory].title}</h3>
                <p className="text-gray-400 text-sm">{skills[activeCategory].items.length} technologies</p>
              </div>
            </div>
          ...
          <AnimatePresence mode="wait" custom={swipeDirection}>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className={`p-2.5 rounded-xl bg-gradient-to-r ${skills[activeCategory].gradient}`}>
                  {skills[activeCategory].icon}
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white">{skills[activeCategory].title}</h3>
                  <p className="text-gray-400 text-xs">{skills[activeCategory].items.length} technologies</p>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-white/10 text-center">
                <p className="text-xs text-gray-500">← Swipe to change category →</p>
              </div>
            </motion.div>
          </AnimatePresence>
        </div>
...
function SkillCard({ skill, index }: { skill: { name: string; icon: string; level: number; color: string }; index: number }) {
  ...
  return (
    ...
    >
      <div className="glass-card p-4 text-center hover:!bg-white/10 hover:!border-white/20 transition-all duration-300 hover:-translate-y-2">
        {/* Icon container */}
        ...
        {/* Skill name */}
        <h4 className="text-sm font-medium text-white mb-2 truncate">{skill.name}</h4>
        ...
        <motion.div
          ...
          className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 rounded bg-black/80 text-xs text-white whitespace-nowrap"
        >
